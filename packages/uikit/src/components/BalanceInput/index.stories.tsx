...
export const SiwtchUnits: React.FC = () => {
  const CAKE_PRICE = 69;
  const [editingUnit, setEditingUnit] = useState<"DEFC" | "USD">("DEFC");
  const conversionUnit = editingUnit === "DEFC" ? "USD" : "DEFC";
  ...
  const switchEditingUnits = () => {
    const editingUnitAfterChange = editingUnit === "DEFC" ? "USD" : "DEFC";
  ...
  const handleCakeChange = (input: string) => {
    ...
    if (editingUnit === "DEFC") {
      ...
      setValues({
        DEFC: Number.isNaN(inputAsFloat) ? "" : `${inputAsFloat / CAKE_PRICE}`,
        USD: input,
      });
    }
  };
